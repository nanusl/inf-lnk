...
public abstract class NettyAbstractRemotingService {
    ...
    private void receivedCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        ...
        final NettyCommandProcessor nettyCommandProcessor = pair.getKey();
        if (nettyCommandProcessor.tryAcquireFailure(3000L)) {
        ...
        Runnable task = new Runnable() {
            ...
            public void run() {
                try {
                    final RemotingCommand response = nettyCommandProcessor.processCommand(ctx, cmd);
                    if (!cmd.isOneway()) {
                        if (response != null) {
